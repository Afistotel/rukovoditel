...
while($item = db_fetch_array($items_query))
{
  ...
  $html_fields = '';  
  $comments_fields_query = db_query("select f.*,ch.fields_value, fr.sort_order as form_rows_sort_order,right(f.forms_rows_position,1) as forms_rows_pos  from app_comments_history ch, app_fields f left join app_forms_rows fr on fr.id=LEFT(f.forms_rows_position,length(f.forms_rows_position)-2) left join app_forms_tabs t on f.forms_tabs_id=t.id where comments_id='" . db_input($item['id']) . "' and f.id=ch.fields_id order by t.sort_order, t.name, fr.sort_order, forms_rows_pos, f.sort_order, f.name");
