...
class related_records
{
    ...
    function __construct($entities_id, $items_id)
    {
        ...
        if(($role = user_roles::getRolseAccessSchema($this->entities_id, $this->items_id)))
        {            
            $this->current_entities_access_schema = $role;
        }
    }   
    ...
    function render_as_single_list($as_single_list = true)
    {
        ...
        while ($field = db_fetch_array($fields_query))
        {
            ...
            if(($role = user_roles::getRolseAccessSchema($this->entities_id, $this->items_id, 'roles_entities_access')) and isset($role[$this->cfg->get('entity_id')]))
            {                
                //print_rr($role[$this->cfg->get('entity_id')]);
                $this->entities_access_schema = $role[$this->cfg->get('entity_id')];
            }
