...
class fieldtype_dropdown_multilevel
{
    ...
    function render($field, $obj, $params = array())
    {
        ...
        if($cfg->get('use_global_list') > 0)
        {
            ...
            $choices_query = db_query("select * from app_global_lists_choices where lists_id = '" . db_input($cfg->get('use_global_list')) . "' and parent_id=0 and (is_active=1 " . (strlen($obj['field_' . $field['id']]) ? " or id in (" . implode(',', array_map(function ($v)
                            {
                                return (int) $v;
                            }, explode(',', $obj['field_' . $field['id']]))) . ")" : '') . ") order by sort_order, name");
        ...
        else
        {
            ...
            $choices_query = db_query("select * from app_fields_choices where fields_id = '" . db_input($field['id']) . "' and parent_id=0 and (is_active=1 " . (strlen($obj['field_' . $field['id']]) ? " or id in (" . implode(',', array_map(function ($v)
                            {
                                return (int) $v;
                            }, explode(',', $obj['field_' . $field['id']]))) . ")" : '') . ") order by sort_order, name");
            while($v = db_fetch_array($choices_query))
