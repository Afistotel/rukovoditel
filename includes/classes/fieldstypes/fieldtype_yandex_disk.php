...
require('includes/libs/FileStorage/YandexDisk/yandex-master/autoload.php');

use Yandex\Disk\DiskClient;

class fieldtype_yandex_disk
{
    ...
    {
        $this->options = array('title' => TEXT_FIELDTYPE_YANDEX_DISK_TITLE);
   ...
   public function getClassName()
   {
    ...
    function get_configuration()
    {
        ...
        $tooltip = '<a href="https://yandex.ru/dev/disk/api/concepts/quickstart.html?lang=ru#quickstart__oauth" target="_balnk" class="btn btn-default">' . TEXT_GET_VALUE . '</a>';
        $cfg[TEXT_SETTINGS][] = array('title' => TEXT_CLIENTID , 'name' => 'client_id', 'type' => 'input', 'tooltip' =>  $tooltip, 'params' => array('class' => 'form-control  required'));                
        
        $tooltip = '
            <a href="https://oauth.yandex.ru/authorize?response_type=token&client_id=" target="_balnk" onClick="return yandex_disk_get_app_code(this)" target="_balnk" class="btn btn-default">' . TEXT_GET_VALUE . '</a>
            <script>
                function yandex_disk_get_app_code(link)
                {
                    let client_id = $("#fields_configuration_client_id").val()
                    
                    if(client_id.length==0)
                    {
                        var validator = $( "#fields_form" ).validate();
                        validator.element( "#fields_configuration_client_id" );                        
                        return false
                    }
                    else
                    {
                        link.href = link.href+client_id
                        return true
                    }
                }           
            </script>     
            ';
        $cfg[TEXT_SETTINGS][] = array('title' => TEXT_OAUTH_TOKEN , 'name' => 'token', 'type' => 'input', 'tooltip' =>  $tooltip, 'params' => array('class' => 'form-control  required'));                
    ...
    function render($field, $obj, $params = array())
    {
        ...
        else
        {
            ...
            {
                $attachments_preview_html = input_hidden_tag('fields[' . $field['id'] . ']', '', array('class' => 'field_' . $field['id'] . ' required'));
            }
        }
        ...
        {            
            ...
            if($file = db_fetch_array($file_info))
            {
                ...
                {      
                    ...
                    $folder = $cfg->get('root_folder') . '/' . $file['folder'];
                    ...
                    try 
                    {
                        ...
                        $disk->setAccessToken($cfg->get('token'));
                        ...
                        foreach(explode('/',$folder) as $name)
                        {
                            ...
                            try
                            {
                                $p = $disk->directoryContents('/' . implode('/', $dir) );
                            }
                            catch (Exception $e) 
                            {
                                if($e->getCode()=='404')
                                {
                                    $disk->createDirectory('/' . implode('/', $dir));
                                }
                            }                                                                                    
                        }
                        ...
                        {							
                            ...
                            echo 'YANDEX DISK ERROR ' . $e->getCode() . ': ' . $e->getMessage();
                        }
    ...
    function download($entity_id,$item_id, $field_id, $file_id)
    {
        ...
        if($file = db_fetch_array($file_query))
        {
            ...
            try{
                $result = $disk->getFile($path);
                
                
                if(in_array($result['headers']['content-type'],['application/pdf','image/jpeg', 'image/png', 'image/gif']))
                {
